...
int main()
{
    ...
    while (looping_count < LOOPING_LIMIT) {
        ...
        if (world_rank != 0) {
            MPI_Recv(&looping_count, 1, MPI_INT, world_rank - 1, 0,
        ...
        MPI_Send(&looping_count, 1, MPI_INT, (world_rank + 1) % world_size,
                 0, MPI_COMM_WORLD);

        // If you are process 0, receive the loop counter from the last
        // process in the world
        if (world_rank == 0) {
            MPI_Recv(&looping_count, 1, MPI_INT, world_size - 1, 0,
                     MPI_COMM_WORLD, MPI_STATUS_IGNORE);
